"""
A3C policy for the industrial inventory management RL project.

Uses Stable-Baselines3's A2C (the synchronous variant of the same
actor-critic algorithm) driven across N_WORKERS parallel vectorized
environments via make_vec_env, following the same interface as ppo.py so
either policy can be dropped straight into evaluation.py's run_policy() slot.
"""
...
MODEL_PATH = Path(__file__).with_name("a3c_model.zip")
...
def train_a3c(
    ...
    env = make_vec_env(
        lambda: InventoryGymWrapper(student_config, scenario_mode="random", domain_randomization=True),
        n_envs=n_workers,
        seed=seed,
    )
...
def save_model(model: A2C, path: Path = MODEL_PATH) -> None:
    model.save(path)


def load_model(path: Path = MODEL_PATH) -> A2C:
    global _MODEL
    _MODEL = A2C.load(path)
    return _MODEL
...
if __name__ == "__main__":
    ...
    model = train_a3c(student_config=student_config)
    save_model(model, MODEL_PATH)

    print()
    print("Training complete.")
    print(f"Saved A3C model to: {MODEL_PATH}")